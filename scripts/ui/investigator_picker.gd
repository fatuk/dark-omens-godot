...
const CARD_TEX_SIZE  := Vector2i(512, 512)
...
func _build_carousel() -> Control:
	...
	# Стрелки — обычные 2D-кнопки поверх 3D-вьюпорта.
	_prev_btn = _make_arrow_btn("‹")
	_prev_btn.set_anchors_and_offsets_preset(Control.PRESET_LEFT_WIDE)
	_prev_btn.anchor_right = 0.0
	_prev_btn.offset_right = 64.0
	_prev_btn.pressed.connect(_on_prev)
	area.add_child(_prev_btn)

	_next_btn = _make_arrow_btn("›")
	_next_btn.set_anchors_and_offsets_preset(Control.PRESET_RIGHT_WIDE)
	_next_btn.anchor_left = 1.0
	_next_btn.offset_left = -64.0
...
func _make_card_ui_subviewport() -> SubViewport:
	...
	quote_lbl.add_theme_color_override("font_color", Color(0.92, 0.88, 0.78, 0.95))
...
func _populate_card(phys_idx: int, inv: Dictionary) -> void:
	...
	# Godot переводит автоматически. tr() оборачиваем в кавычки-«ёлочки».
	var quote_key: String = inv.get("quote", "")
	if phys_idx < _slot_quotes.size():
		var ql: Label = _slot_quotes[phys_idx]
		ql.text = "«%s»" % tr(quote_key) if not quote_key.is_empty() else ""
