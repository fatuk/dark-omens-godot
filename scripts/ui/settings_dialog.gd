...
func _build_server_section() -> void:
	_section_header("SECTION_SERVER")
	...
	_vbox.add_child(row[0])


func _build_display_section() -> void:
	_section_header("SECTION_DISPLAY")
	...
	fs_row.add_child(_plain_label("SETTINGS_FULLSCREEN"))
	...
	fx_row.add_child(_plain_label("SETTINGS_FX_OLD_FILM"))


func _build_audio_section() -> void:
	_section_header("SECTION_AUDIO")
...
func _build_language_section() -> void:
	...
	_vbox.add_child(row)
	row.add_child(_form_label(tr("SETTINGS_LANGUAGE") + ":"))
...
func _section_header(key: String) -> void:
	var lbl := Label.new()
	lbl.text = key
