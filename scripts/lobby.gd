...
const COLOR_GREEN        := Color(0.20, 0.70, 0.35)
...
func _build_ui() -> void:
	...
	_room_label.add_theme_color_override("font_color", COLOR_DIM)
	_update_room_label()
	...
	list_header.add_theme_color_override("font_color", COLOR_DIM)
	panel_vbox.add_child(list_header)

	_add_separator(panel_vbox)
	...
	var back_btn := _make_button("  ПОКИНУТЬ", COLOR_RED)
	back_btn.pressed.connect(_on_back_pressed)
	buttons_row.add_child(back_btn)

	if _nm.is_host():
		var delete_btn := _make_button("  ЗАКРЫТЬ КОМНАТУ", COLOR_RED)
		delete_btn.pressed.connect(_on_delete_room_pressed)
		buttons_row.add_child(delete_btn)

	_ready_button = _make_button("  ГОТОВ", COLOR_PANEL_BORDER)
	...
	if _nm.is_host():
		_start_button = _make_button("  НАЧАТЬ ИГРУ", COLOR_GOLD)
...
func _on_player_left_relay(player_id: String, new_host_id: String) -> void:
	...
	if new_host_id == _nm.my_id:
		...
		if not is_instance_valid(_start_button):
			# Добавить кнопку старта динамически
			pass
	_refresh_start_button()
...
func _on_reconnected() -> void:
	...
	# Обновляем список игроков (некоторые могли уйти)
...
func _on_rejoin_failed() -> void:
	# Комната перестала существовать (сервер перезапустился)
...
func _on_connection_lost() -> void:
	# Исчерпали все попытки переподключения
...
func _on_room_deleted(reason: String) -> void:
	# Нас выкинули — идём в меню (оно само покажет список комнат)
...
func _add_player_row(id: String, info: Dictionary) -> void:
	...
	var _is_host_row: bool = (id == _nm.my_id and _nm.is_host()) or \
		(not _nm.players.is_empty() and id == _nm.players.keys()[0] and _nm.is_host())
	# Хост — тот, чей ID совпадает с нашим при is_host, иначе определяем по порядку
	# Проще: в relay-мире хост = _nm.is_host() при id == _nm.my_id
	var crown_char: String = "♛" if (id == _nm.my_id and _nm.is_host()) else "◆"
	var crown_color: Color = COLOR_GOLD if (id == _nm.my_id and _nm.is_host()) else COLOR_DIM

	var crown := Label.new()
	crown.text = crown_char
	crown.add_theme_font_size_override("font_size", 14)
	crown.add_theme_color_override("font_color", crown_color)
	...
	var tag := Label.new()
	if id == _nm.my_id and _nm.is_host():
		tag.text = "Ведущий"
		tag.add_theme_color_override("font_color", COLOR_GOLD)
	elif info.get("ready", false):
		tag.text = "Готов"
		tag.add_theme_color_override("font_color", COLOR_GREEN)
	else:
		tag.text = "Ожидает..."
...
func _refresh_start_button() -> void:
	if not _nm.is_host() or not is_instance_valid(_start_button):
		if is_instance_valid(_status_label):
			if not _nm.is_host():
				...
				_status_label.modulate = COLOR_DIM
		return
	...
	if non_host < 1:
		...
		_status_label.modulate = COLOR_WARN
	else:
		_status_label.text = "Игроков: %d  •  Можно начинать!" % _nm.players.size()
		_status_label.modulate = COLOR_GREEN
...
func _show_reconnect_overlay(text: String) -> void:
	...
	(_reconnect_overlay as ColorRect).color = Color(0.0, 0.0, 0.0, 0.75)
	...
	_reconnect_overlay.add_child(lbl)
